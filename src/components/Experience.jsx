        ...
        <div className="glass p-6 md:p-8 rounded-3xl border-white/10 bg-white/5 relative overflow-hidden transition-all duration-300 hover:shadow-[0_0_40px_rgba(168,85,247,0.1)] hover:bg-white/10 hover:-translate-y-1">
          ...
          <div className="flex flex-col sm:flex-row gap-5 mb-4">
            ...
            <div className="flex-1 flex flex-col gap-1 min-w-0">
              <h3 className="text-xl md:text-2xl font-bold text-white whitespace-nowrap truncate">
                {exp.role}
              </h3>
              <h4 className="text-lg text-purple-300 font-medium">
                {exp.company}
              </h4>
          ...
          >
            <ul className="space-y-3 pl-1">
              {exp.responsibilities.map((item, i) => (
                <li
                  key={i}
                  className="text-sm md:text-base text-gray-300/90 leading-relaxed flex gap-3"
                >
          ...
          <div
            className={`w-full flex items-center justify-center gap-2 text-[10px] text-gray-600 uppercase tracking-widest transition-all duration-300 ${isOpen ? "mt-4" : "mt-2"}`}
          >
...
const Experience = () => {
  return (
    <section
      id="experience"
      className="flex w-full flex-col items-center gap-12 py-20 relative"
    >
      <motion.div
